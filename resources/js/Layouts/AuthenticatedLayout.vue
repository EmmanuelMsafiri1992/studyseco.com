...
const user = page.props.auth?.user || { name: 'Guest', role: 'guest', profile_photo_url: null };
</script>
...
<template>
    <div class="flex h-screen bg-gradient-to-br from-slate-50 to-blue-50 font-sans text-slate-800">
            ...
            <nav class="px-4 py-6 space-y-2">
                ...
                <template v-if="user.role === 'admin'">
                    <Link href="/admin/site-content" 
                          :class="page.url.startsWith('/admin/site-content') ? 'text-slate-700 bg-indigo-50 border border-indigo-100' : 'text-slate-600'"
                          class="flex items-center px-4 py-3 rounded-xl transition-all duration-200 hover:bg-slate-50 hover:text-slate-800">
                        <svg class="h-5 w-5 mr-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="pt-4 mt-4 border-t border-slate-200">
                    <Link href="/settings" 
                          :class="page.url.startsWith('/settings') ? 'text-slate-700 bg-indigo-50 border border-indigo-100' : 'text-slate-600'"
                          class="flex items-center px-4 py-3 rounded-xl transition-all duration-200 hover:bg-slate-50 hover:text-slate-800">
                        <svg class="h-5 w-5 mr-4" :class="page.url.startsWith('/settings') ? 'text-indigo-600' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="flex-1 flex flex-col">
            ...
            <header class="h-20 bg-white/70 backdrop-blur-xl border-b border-slate-200/50 px-8 flex items-center justify-between relative z-50">
                <div>
                    ...
                    <p class="text-slate-500 text-sm">
                        <slot name="subtitle">Manage and configure system settings</slot>
                    </p>
                </div>
                <div class="flex items-center space-x-4">
                    <div class="relative">
                        <input type="text" placeholder="Search..." class="bg-slate-100/70 backdrop-blur-sm px-4 py-3 pl-10 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:bg-white w-80 transition-all duration-200">
            ...
            <header class="bg-white shadow" v-if="$slots.header" style="display: none;">
