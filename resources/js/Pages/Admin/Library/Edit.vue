...
const form = useForm({
    ...
    grade_level: props.resource.grade_level || '',
    exam_board: props.resource.exam_board || '',
    year: props.resource.year || new Date().getFullYear(),
    file: null,
    thumbnail: null,
    access_permissions: props.resource.access_permissions || {
        roles: [],
        users: []
    },
    is_protected: props.resource.is_protected || false,
    protection_settings: props.resource.protection_settings || {
...
const submit = () => {
    form.put(route('admin.library.update', props.resource.id), {
        onSuccess: () => {
...
const showPastPaperFields = ref(props.resource.type === 'past_paper');
const showBookFields = ref(props.resource.type === 'book');
...
<template>
    ...
    <DashboardLayout :title="`Edit: ${resource.title}`" subtitle="Update resource information and files">
        <div class="max-w-4xl mx-auto">
            ...
            <form @submit.prevent="submit" class="space-y-8">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                    <div class="flex items-center space-x-3 mb-6">
                        <span class="text-2xl">📁</span>
                        <h3 class="text-lg font-semibold text-slate-900">Current File</h3>
                    </div>

                    <div class="bg-slate-50 rounded-xl p-4 mb-6">
                        <div class="flex items-center justify-between">
                            <div>
                                <div class="font-medium text-slate-900">{{ resource.file_path || 'No file' }}</div>
                                <div class="text-sm text-slate-500">{{ formatFileSize(resource.file_size) }}</div>
                            </div>
                            <div class="text-sm text-slate-500">
                                {{ resource.file_type || 'Unknown type' }}
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                    <div class="flex items-center space-x-3 mb-6">
                        <span class="text-2xl">📎</span>
                        <h3 class="text-lg font-semibold text-slate-900">Replace Files (Optional)</h3>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-2">Replace Main File</label>
                            <input
                                type="file"
                                ...
                                class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                            />
                            <p class="text-sm text-slate-500 mt-1">Leave blank to keep current file</p>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-2">Replace Thumbnail</label>
                            <input
                                type="file"
                                ...
                                class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                            />
                            <p class="text-sm text-slate-500 mt-1">Leave blank to keep current thumbnail</p>
