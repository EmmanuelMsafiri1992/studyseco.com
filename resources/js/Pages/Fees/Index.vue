...
const fees = ref(props.fees.length > 0 ? props.fees : [
    { studentId: 'S1001', name: 'Michael Chen', amountDue: 500, status: 'paid', dueDate: '2023-09-01' },
    { studentId: 'S1002', name: 'Emma Rodriguez', amountDue: 600, status: 'overdue', dueDate: '2023-08-15' },
    { studentId: 'S1003', name: 'James Thompson', amountDue: 450, status: 'pending', dueDate: '2023-09-15' },
]);
    ...
    <div class="flex h-screen bg-gradient-to-br from-slate-50 to-blue-50 font-sans text-slate-800">
        ...
        <div class="w-72 bg-white/80 backdrop-blur-xl border-r border-slate-200/50 flex-shrink-0 shadow-xl">
            ...
            <nav class="px-4 py-6 space-y-2">
                ...
                <Link href="/fees" class="flex items-center px-4 py-3 text-slate-700 bg-indigo-50 rounded-xl border border-indigo-100 transition-all duration-200 hover:bg-indigo-100">
        ...
        <div class="flex-1 flex flex-col">
            ...
            <header class="h-20 bg-white/70 backdrop-blur-xl border-b border-slate-200/50 px-8 flex items-center justify-between relative z-50">
                <div>
                    ...
                    <p class="text-slate-500 text-sm">Manage student fees</p>
                </div>
            ...
            <main class="flex-1 overflow-y-auto p-8 space-y-8 relative">
                ...
                <div class="bg-white/80 backdrop-blur-sm rounded-3xl p-6 shadow-xl border border-slate-200/50">
                    <div class="flex flex-col md:flex-row gap-4 items-center justify-between">
                        <div class="flex-1 max-w-md">
                            <div class="relative">
                                <input v-model="searchQuery" type="text" placeholder="Search fees..." class="w-full bg-slate-100/70 backdrop-blur-sm px-4 py-3 pl-10 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:bg-white transition-all duration-200">
                                <svg class="absolute left-3 top-3.5 h-4 w-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path>
                                </svg>
                            </div>
                        ...
                        <div class="flex items-center space-x-4">
                            ...
                            <Link href="/fees/create" class="bg-gradient-to-r from-indigo-500 to-purple-600 text-white px-6 py-3 rounded-2xl font-semibold hover:from-indigo-600 hover:to-purple-700 transition-all duration-200 shadow-lg hover:shadow-xl transform hover:-translate-y-1">
                ...
                <div class="bg-white/80 backdrop-blur-sm rounded-3xl shadow-xl border border-slate-200/50 overflow-hidden">
                    <div class="p-6 border-b border-slate-200/50">
                        <h2 class="text-xl font-bold text-slate-800">All Fees</h2>
                        <p class="text-sm text-slate-500 mt-1">{{ filteredFees.length }} fees found</p>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="w-full">
                            ...
                            <tbody class="divide-y divide-slate-200/50">
                                <tr v-for="fee in filteredFees" :key="fee.studentId" class="hover:bg-slate-50/50 transition-colors duration-150">
                                    <td class="px-6 py-4 text-sm font-medium text-slate-800">{{ fee.studentId }}</td>
                                    <td class="px-6 py-4 text-sm text-slate-800">{{ fee.name }}</td>
                                    <td class="px-6 py-4 text-sm text-slate-600">${{ fee.amountDue }}</td>
                                    <td class="px-6 py-4">
                                        <span :class="{
                                    ...
                                    <td class="px-6 py-4 text-sm text-slate-600">{{ fee.dueDate }}</td>
                                    <td class="px-6 py-4">
                                        <div class="flex items-center space-x-2">
                                            <button class="text-indigo-600 hover:text-indigo-800 text-sm font-medium">Send Reminder</button>
                                            <button class="text-amber-600 hover:text-amber-800 text-sm font-medium">Mark Paid</button>
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
                    ...
                    <div class="px-6 py-4 border-t border-slate-200/50">
                        <div class="flex items-center justify-between">
                            <p class="text-sm text-slate-600">Showing 1 to {{ filteredFees.length }} of {{ filteredFees.length }} results</p>
                            <div class="flex items-center space-x-2">
