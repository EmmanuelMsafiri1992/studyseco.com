    ...
    <div class="flex h-screen bg-gradient-to-br from-slate-50 to-blue-50 font-sans text-slate-800">
        ...
        <div class="flex-1 flex flex-col">
            ...
            <header class="h-20 bg-white/70 backdrop-blur-xl border-b border-slate-200/50 px-8 flex items-center justify-between relative z-50">
                <div>
                    <h1 class="text-2xl font-bold text-slate-800">Complaints Management</h1>
                    <p class="text-slate-500 text-sm">View and resolve complaints</p>
                </div>
                ...
                <div class="bg-white/80 backdrop-blur-sm rounded-3xl p-6 shadow-xl border border-slate-200/50">
                    <div class="flex flex-col md:flex-row gap-4 items-center justify-between">
                        <div class="flex-1 max-w-md">
                            <div class="relative">
                                <input v-model="searchQuery" type="text" placeholder="Search complaints..." class="w-full bg-slate-100/70 backdrop-blur-sm px-4 py-3 pl-10 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:bg-white transition-all duration-200">
                                <svg class="absolute left-3 top-3.5 h-4 w-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path>
                                </svg>
                            </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full">
                            <thead class="bg-slate-50/50">
                                <tr>
                                    <th class="px-6 py-4 text-left text-sm font-semibold text-slate-600">Ticket ID</th>
                            ...
                            <tbody class="divide-y divide-slate-200/50">
                                <tr v-for="complaint in filteredComplaints" :key="complaint.id" class="hover:bg-slate-50/50 transition-colors duration-150">
                                    ...
                                    <td class="px-6 py-4 text-sm text-slate-600">{{ complaint.user }}</td>
                                    <td class="px-6 py-4 text-sm text-slate-600">{{ complaint.issue }}</td>
                                    <td class="px-6 py-4">
                                        <span :class="complaint.status === 'open' ? 'bg-red-100 text-red-800' : 'bg-emerald-100 text-emerald-800'" class="px-3 py-1 text-xs font-semibold rounded-full">
                                            {{ complaint.status }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 text-sm text-slate-600">{{ complaint.date }}</td>
                                    <td class="px-6 py-4">
                                        <div class="flex items-center space-x-2">
                                            <button class="text-indigo-600 hover:text-indigo-800 text-sm font-medium">View</button>
                                            <button class="text-amber-600 hover:text-amber-800 text-sm font-medium">Resolve</button>
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
