...
// Mock data fallback
const payments = ref(props.payments.length > 0 ? props.payments : [
    { id: 'P001', studentId: 'S1001', amount: 500, date: '2023-08-20', method: 'Credit Card' },
    { id: 'P002', studentId: 'S1002', amount: 600, date: '2023-08-10', method: 'Bank Transfer' },
    { id: 'P003', studentId: 'S1003', amount: 450, date: '2023-08-15', method: 'Cash' },
]);
...
const filteredPayments = computed(() => {
    return payments.value.filter(payment => {
        return payment.studentId.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
    ...
    <div class="flex h-screen bg-gradient-to-br from-slate-50 to-blue-50 font-sans text-slate-800">
        ...
        <div class="w-72 bg-white/80 backdrop-blur-xl border-r border-slate-200/50 flex-shrink-0 shadow-xl">
            ...
            <nav class="px-4 py-6 space-y-2">
                ...
                <Link href="/payments" class="flex items-center px-4 py-3 text-slate-700 bg-indigo-50 rounded-xl border border-indigo-100 transition-all duration-200 hover:bg-indigo-100">
                ...
                <Link href="/complaints" class="flex items-center px-4 py-3 text-slate-600 rounded-xl transition-all duration-200hover:bg-slate-50 hover:text-slate-800">
                    <svg class="h-5 w-5 mr-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="flex-1 flex flex-col">
            ...
            <header class="h-20 bg-white/70 backdrop-blur-xl border-b border-slate-200/50 px-8 flex items-center justify-between relative z-50">
                <div>
                    ...
                    <p class="text-slate-500 text-sm">View all payments</p>
                </div>
            ...
            <main class="flex-1 overflow-y-auto p-8 space-y-8 relative">
                ...
                <div class="bg-white/80 backdrop-blur-sm rounded-3xl p-6 shadow-xl border border-slate-200/50">
                    <div class="flex flex-col md:flex-row gap-4 items-center justify-between">
                        <div class="flex-1 max-w-md">
                            <div class="relative">
                                <input v-model="searchQuery" type="text" placeholder="Search payments..." class="w-full bg-slate-100/70 backdrop-blur-sm px-4 py-3 pl-10 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:bg-white transition-all duration-200">
                                <svg class="absolute left-3 top-3.5 h-4 w-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path>
                                </svg>
                            </div>
                ...
                <div class="bg-white/80 backdrop-blur-sm rounded-3xl shadow-xl border border-slate-200/50 overflow-hidden">
                    <div class="p-6 border-b border-slate-200/50">
                        <h2 class="text-xl font-bold text-slate-800">All Payments</h2>
                        <p class="text-sm text-slate-500 mt-1">{{ filteredPayments.length }} payments found</p>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="w-full">
                            <thead class="bg-slate-50/50">
                                <tr>
                                    <th class="px-6 py-4 text-left text-sm font-semibold text-slate-600">Payment ID</th>
                                    <th class="px-6 py-4 text-left text-sm font-semibold text-slate-600">Student ID</th>
                                    <th class="px-6 py-4 text-left text-sm font-semibold text-slate-600">Amount</th>
                            ...
                            <tbody class="divide-y divide-slate-200/50">
                                <tr v-for="payment in filteredPayments" :key="payment.id" class="hover:bg-slate-50/50 transition-colors duration-150">
                                    <td class="px-6 py-4 text-sm font-medium text-slate-800">{{ payment.id }}</td>
                                    <td class="px-6 py-4 text-sm text-slate-600">{{ payment.studentId }}</td>
                                    <td class="px-6 py-4 text-sm text-slate-600">${{ payment.amount }}</td>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center space-x-2">
                                            <button class="text-indigo-600 hover:text-indigo-800 text-sm font-medium">View Receipt</button>
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
                    ...
                    <div class="px-6 py-4 border-t border-slate-200/50">
                        <div class="flex items-center justify-between">
                            <p class="text-sm text-slate-600">Showing 1 to {{ filteredPayments.length }} of {{ filteredPayments.length }} results</p>
                            <div class="flex items-center space-x-2">
